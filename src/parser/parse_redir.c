...
/*   Updated: 2025/10/11 10:39:54 by nnishiya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_redir_struct(t_redir *r, t_tokentype redir_type,
		t_token *word_tok)
{
	...
	{
		if (word_tok->u_u.arg.parts && word_tok->u_u.arg.parts->quote != Q_NONE)
		{
			r->quoted_heredoc = 1;
		}
	}
...
static int	append_redir_to_node(t_node *node, t_redir *r)
{
	...
	m = node->u_as.cmd.n_redirs;
	tmp = ft_realloc(node->u_as.cmd.redirs, sizeof(t_redir) * m, sizeof(t_redir)
			* (m + 1));
	if (!tmp)
	{
		perror("realloc");
	...
	node->u_as.cmd.redirs = tmp;
	node->u_as.cmd.redirs[m] = *r;
	node->u_as.cmd.n_redirs++;
	return (0);
}
